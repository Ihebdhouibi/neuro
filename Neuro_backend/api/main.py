...
# Load environment variables BEFORE importing build_scorecard
# This ensures API_KEY and other env vars are available when build_scorecard initializes
load_dotenv()

# TODO: build_scorecard module is missing - commenting out for now
# from build_scorecard import load_course_reference, call_openai_extract, build_eleven_row_dataframe
...
COURSE_REFERENCE_PATH = str(Path(__file__).parent.parent / "sample_schema.json")
# Response models
...
class ParseResponse(BaseModel):
    ...
    extract: Optional[dict] = None       # 👈 structured JSON from OpenAI
    dataframe: Optional[List[List[str]]] = None  # 👈 DataFrame as 2D list
...
# TODO: Disabled due to missing build_scorecard module
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning(f"⚠️ Failed to initialize MonkeyOCR model: {e}")
        logger.warning("⚠️ OCR features will be disabled. Server will run without OCR functionality.")
...
# ✅ Allow React frontend (localhost:5173) to talk to FastAPI backend
...
async def async_parse_file(input_file_path: str, output_dir: str, split_pages: bool = False, max_pages: Optional[int] = None):
    ...
    def create_dataset_sync():
        ...
        if file_extension == ".pdf":
            ...
            return ImageDataset(file_bytes, file_extension=file_extension)
...
async def parse_document_internal(file: UploadFile, split_pages: bool = False, max_pages: Optional[int] = None):
    ...
    try:
        ...
        try:
            ...
            # 🔎 Extract main .md content if exists
            md_content, extract, df_json = None, None, None
            
            # Find markdown files - prioritize first page when split, otherwise any .md file
            md_files = [f for f in files if f.endswith(".md")]
            if md_files:
                # When pages are split, prefer page_0 files, otherwise use first found
                if split_pages:
                    page_0_files = [f for f in md_files if f.startswith("page_0")]
                    target_file = page_0_files[0] if page_0_files else md_files[0]
                else:
                    target_file = md_files[0]
                
                md_path = os.path.join(result_dir, target_file)
                try:
                    md_content = Path(md_path).read_text(encoding="utf-8")
                    logger.info(f"Processing markdown file: {md_path}")

                    # TODO: Scorecard pipeline disabled - using basic extraction from 1.py instead
                    ...
                    # Update ocr_raw_text with full content if extraction succeeded
                    if extract and md_content:
                        extract["ocr_raw_text"] = md_content
                    
                    # Enrich extract with medical list information from database
                    if extract:
                        try:
                            from database import AsyncSessionLocal
                            from api.crud import search_medical_lists_by_codes
                            
                            # Get acts_prescribed codes
                            acts_prescribed = extract.get("orthoptic_care", {}).get("acts_prescribed", [])
                            
                            if acts_prescribed:
                                async with AsyncSessionLocal() as db_session:
                                    # Search for medical lists by codes
                                    medical_lists = await search_medical_lists_by_codes(db_session, acts_prescribed)
                                    
                                    # Create a mapping of code -> medical list details
                                    medical_list_map = {ml.code: ml.to_dict() for ml in medical_lists}
                                    
                                    # Enrich acts_prescribed with full details
                                    enriched_acts = []
                                    for code in acts_prescribed:
                                        act_info = {
                                            "code": code,
                                            "label": None,
                                            "price": None,
                                        ...
                                        # If we found a match in the database, add the details
                                        if code in medical_list_map:
                                            ml = medical_list_map[code]
                                            act_info["label"] = ml.get("label")
                                            act_info["price"] = ml.get("price")
                                            act_info["category"] = ml.get("category")
                                        
                                        enriched_acts.append(act_info)
                                    
                                    # Update the extract with enriched acts
                                    if "orthoptic_care" not in extract:
                                        extract["orthoptic_care"] = {}
                                    extract["orthoptic_care"]["acts_prescribed_details"] = enriched_acts
                                    
                        except Exception as enrich_error:
                            logger.warning(f"Failed to enrich extract with medical lists: {enrich_error}")
                            # Continue without enrichment if database lookup fails
                    
                    df_json = df.values.tolist() if df is not None else None
                    logger.info(f"Extraction completed. Extract keys: {list(extract.keys()) if extract else 'None'}")

                except Exception as e:
                    logger.error(f"Failed to process markdown file {md_path}: {e}", exc_info=True)
...
async def generate_prescription_endpoint(request: PrescriptionRequest):
    ...
    try:
        import sys
        import os
        # Add parent directory to path to import prescription_generator
