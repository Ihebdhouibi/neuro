...
# Medical List CRUD
async def create_medical_list(db: AsyncSession, medical_list: schemas.MedicalListCreate) -> models.MedicalList:
    """Create a new medical list item"""
    medical_list_data = medical_list.model_dump()
    db_medical_list = models.MedicalList(**medical_list_data)
    db.add(db_medical_list)
    await db.commit()
    await db.refresh(db_medical_list)
    return db_medical_list

async def get_medical_list(db: AsyncSession, medical_list_id: int) -> Optional[models.MedicalList]:
    """Get a medical list item by ID"""
    result = await db.execute(
        select(models.MedicalList).filter(models.MedicalList.id == medical_list_id)
    )
    return result.scalar_one_or_none()

async def get_medical_list_by_code(db: AsyncSession, code: str) -> Optional[models.MedicalList]:
    """Get a medical list item by code"""
    result = await db.execute(
        select(models.MedicalList).filter(models.MedicalList.code == code, models.MedicalList.is_active == True)
    )
    return result.scalar_one_or_none()
...
) -> List[models.MedicalList]:
    """Get list of medical list items with optional filtering"""
    query = select(models.MedicalList)
    
    if category:
        query = query.filter(models.MedicalList.category == category)
    if is_active is not None:
        query = query.filter(models.MedicalList.is_active == is_active)
    
    query = query.offset(skip).limit(limit).order_by(models.MedicalList.code)
    result = await db.execute(query)
    return result.scalars().all()

async def search_medical_lists_by_codes(
    db: AsyncSession,
    codes: List[str]
) -> List[models.MedicalList]:
    """Search medical list items by multiple codes"""
    if not codes:
        return []
    
    result = await db.execute(
        select(models.MedicalList).filter(
            models.MedicalList.code.in_(codes),
            models.MedicalList.is_active == True
        )
    )
    return result.scalars().all()
...
) -> Optional[models.MedicalList]:
    """Update a medical list item"""
    update_data = medical_list_update.model_dump(exclude_unset=True)
    
    if update_data:
        await db.execute(
            update(models.MedicalList)
            .where(models.MedicalList.id == medical_list_id)
            .values(**update_data)
        )
        await db.commit()
    
    return await get_medical_list(db, medical_list_id)

async def delete_medical_list(db: AsyncSession, medical_list_id: int) -> bool:
    """Delete a medical list item"""
    result = await db.execute(
        delete(models.MedicalList).where(models.MedicalList.id == medical_list_id)
    )
    await db.commit()
    return result.rowcount > 0
...
async def bulk_create_medical_lists(
    ...
    db_medical_lists = [
        models.MedicalList(**ml.model_dump()) for ml in medical_lists
    ]
    db.add_all(db_medical_lists)
    await db.commit()
    for ml in db_medical_lists:
        await db.refresh(ml)
    return db_medical_lists
